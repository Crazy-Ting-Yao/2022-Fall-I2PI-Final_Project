...
static RecArea mapselectionblock[3];
// [HACKATHON 3]
...
static char mapname[3][6]={"TEST","SUS","NTHU"};
static void init() {
    ...
    for(int i=0;i<3;i++){
        mapselectionblock[i].x=SCREEN_W/4*(i+1)-80;
        mapselectionblock[i].y=500;
        mapselectionblock[i].w=160;
        mapselectionblock[i].h=100;
    }
}
...
static void draw() {
    ...
    al_draw_filled_rectangle(SCREEN_W/4*(mapselection+1)-82, 502, SCREEN_W/4*(mapselection+1)+78,598, al_map_rgb(155, 155, 155));
    for(int i=0;i<3;i++){
           // al_draw_filled_rectangle(keys[i].x+2, keys[i].y+2, keys[i].x+keys[i].w-2, keys[i].y+keys[i].h-2, al_map_rgb(155, 155, 155));
        al_draw_rectangle(SCREEN_W/4*(i+1)-80, 500, SCREEN_W/4*(i+1)+80,600, al_map_rgb(255, 255, 255), 4);
        al_draw_text( settingsFont, al_map_rgb(255, 255, 255), SCREEN_W/4*(i+1), 525, ALLEGRO_ALIGN_CENTER, mapname[i]);
    }
...
static void on_mouse_down() {
	if (btnSettings.hovered)
        ...
        for(int i=0;i<3;i++){
            if(pnt_in_rect(mouse_x,mouse_y,mapselectionblock[i])) {
                if(mapselection!=i){
                    mapselection=i;
                }
                break;
